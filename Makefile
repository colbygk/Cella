...
JAVA_SOURCES = Cella.java Diary.java XLevel.java
JAVA_FQ_SOURCES = $(JAVA_SOURCES:%.java=$(SRC_DIR)/%.java)
JAVA_NOLIB_CLASSES = $(patsubst %.java,$(PACKAGE)/%.class,$(JAVA_SOURCES))
JAVA_CLASSES = $(patsubst %.java,$(LIB_DIR)/$(PACKAGE)/%.class,$(JAVA_SOURCES))
TEMPFILE := $(shell mktemp /tmp/manifest.XXXXX)


COMPILE_JAVA_SOURCES = javac -sourcepath $(SRC_DIR) -classpath $(LIB_DIR):$(LOG4J_JARS):$(JUNIT_JAR):$(HAM_JAR) -d $(LIB_DIR) $(JAVA_FQ_SOURCES)
